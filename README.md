...
- **在原处读外文，按 <kbd>⌥</kbd> <kbd>D</kbd>。** 点一段，译文就盖在原文上；按住 <kbd>⇧</kbd> 点一个区域（比如 Slack 的消息列表、网页正文），之后这里的外文会一直显示为译文，跟着内容滚动。指针停在译文上就能看原文，点击和滚动照常落到原来的应用。
...
| 屏幕录制 | 用 <kbd>⌥</kbd> <kbd>S</kbd> 截图翻译；原处的译文用原文的底色，并逐帧跟着滚动 | 截图翻译不可用；原处的译文画在纸片上，滚动时先隐去 |

## 隐私

- API Key 只存在 macOS 的钥匙串里。
- 不保存历史记录，不收集任何数据，翻译和润色的请求只发往你选择的服务商。
- 截图在本机用 Apple 的 Vision 识别，图片不会离开你的 Mac。原处翻译只读取应用通过辅助功能提供的文字，画面只在内存里逐帧比较，不保存、不上传。
...
| <kbd>⌥</kbd> <kbd>D</kbd> | 原处翻译：点一段翻译一次，<kbd>⇧</kbd> 点一个区域一直翻译 |
...
辞达每天检查一次 `https://cida-releases.xuanwo.io/appcast.xml`，有新版本时在面板里给出更新说明，确认后自己下载并安装。检查不附带任何系统信息，可以在设置的「通用」页关掉。
