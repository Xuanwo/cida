...
- The source and result panes are native `NSScrollView`s with the system overlay scroll bar: it appears while scrolling, fades out at rest, and follows the user's scroll-bar preference. The result scroll view spans the pane and insets its text, so both bars sit on the panel's right edge.
